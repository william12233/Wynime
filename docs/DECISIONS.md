...
**Safety:** The listener binds only `127.0.0.1` or `::1`; redirects and child resources remain allowlisted; DNS must resolve only to public addresses; request and response resources are bounded; Set-Cookie and Location are not forwarded; lease close cancels active body subscriptions; Media3 accepts only loopback endpoints. Phase 4 does not sanitize manifests, detect ads or rewrite timelines.
